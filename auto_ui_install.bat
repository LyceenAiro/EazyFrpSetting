...
if "%file3date%" gtr "%file4date%" (
  PySide6-rcc ui/main.qrc -o ui/main_qrc.py
  echo [%date:~0,4%-%date:~5,2%-%date:~8,2% %time:~0,2%:%time:~3,2%:%time:~6,2%]main_qrc.py was update! >> auto_ui_install.txt
  echo [INFO][%time:~0,2%:%time:~3,2%:%time:~6,2%]main_qrc.py was update!
)
@echo off
goto loop
